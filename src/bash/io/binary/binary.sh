...
function binary.is_printable {
	...
	if __command_exists "file"; then
		if [[ "$(file "$1" 2>/dev/null)" =~ .+text$ ]]; then
			result=1
		elif [[ "$(file "$1" 2>/dev/null)" =~ .+text.+ ]]; then
			result=1
		fi
	else
...
function binary.is_html {
	...
	result=1
	if __command_exists "file"; then
		if [[ ! "$(file "$1" 2>/dev/null)" =~ .+HTML\ document.+ ]]; then
			result=0
		fi
	else
...
function binary.is_binary {
	...
	result=1
	if __command_exists "file"; then
		if [[ ! "$(file "$1" 2>/dev/null)" =~ .+executable.+ ]]; then
			result=0
		fi
	else
...
function binary.is_pdf {
	...
	result=1
	if __command_exists "file"; then
		if [[ ! "$(file "$1" 2>/dev/null)" =~ .+PDF\ document.+ ]]; then
			result=0
		fi
	else
...
function binary.is_jpeg {
	...
	result=1
	if __command_exists "file"; then
		if [[ ! "$(file "$1" 2>/dev/null)" =~ .+JPEG.+ ]]; then
			result=0
		fi
	else
...
function binary.is_image {
	...
	result=1
	if __command_exists "file"; then
		if [[ ! "$(file "$1" 2>/dev/null)" =~ .+image\ data.+ ]]; then
			result=0
		fi
	else
...
function binary.is_executable {
	...
	result=0
	if __command_exists "file"; then
		if [[ "$(file "$target" 2>/dev/null)" =~ .+executable.+ ]]; then
			local arch
			arch="$(os.get_architecture)"
			arch="$(strings.replace_char_with "$arch" _ -)"
			if [[ "$(file "$target" 2>/dev/null)" =~ .+$arch.+ ]]; then
				result=1
			fi
		elif [[ "$(file "$target" 2>/dev/null)" =~ .+executable$ ]]; then
			result=1
		fi
	else
...
function binary.hexdump {
	...
	content=""
	if binary.is_binary "$target"; then
		if __command_exists "xxd"; then
			content="$(xxd $target)"
		elif __command_exists "hexdump"; then
