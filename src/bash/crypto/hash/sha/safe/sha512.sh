...
function crypto.hash.sha512sum {
	...
	if __command_exists "shasum"; then
		...
		redirect.text_to_error "$(logging.warning "[shasum] command not available, cannot determine SHA512 hash of $target")"
		exit 1
	fi
	echo $_hash
}
...
function crypto.hash.sha512_224sum {
	...
	if __command_exists "shasum"; then
		...
		redirect.text_to_error "$(logging.warning "[shasum] command not available, cannot determine SHA512/224 hash of $target")"
		exit 1
	fi
	echo $_hash
}
...
function crypto.hash.sha512_256sum {
	...
	if __command_exists "shasum"; then
		...
		redirect.text_to_error "$(logging.warning "[shasum] command not available, cannot determine SHA512/256 hash of $target")"
		exit 1
	fi
	echo $_hash
}
