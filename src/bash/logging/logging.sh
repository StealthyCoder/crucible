...
function logging.info {
	if [ "$(__check_log_level "$(logging.level.info)")" -ne 0 ]; then
...
function logging.debug {
	if [ "$(__check_log_level "$(logging.level.debug)")" -ne 0 ]; then
		_format "$(logging.level.debug)" "$1"
	fi
}

function logging.warning {
	if [ "$(__check_log_level "$(logging.level.warning)")" -ne 0 ]; then
		_format "$(logging.level.warning)" "$1"
	fi
}

function logging.error {
	if [ "$(__check_log_level "$(logging.level.error)")" -ne 0 ]; then
		_format "$(logging.level.error)" "$1"
	fi
}

function logging.success {
	if [ "$(__check_log_level "$(logging.level.success)")" -ne 0 ]; then
...
function logging.set_level {
	if [ "${CRUCIBLE_LOGGING_LEVEL:-xxx__xxx}" == "xxx__xxx" ]; then
